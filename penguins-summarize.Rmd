...
The `tidyselect` grammar can also be used in some places in the contruction of a `gt` table.  For example, we can just show 1 decimal place for the numeric variables.  Other handy format functions that are `tidyselect`-aware are `fmt_currency`, `fmt_date`, `fmt_time`, `fmt_datetime`, `fmt_percent`, and `fmt_markdown`.
